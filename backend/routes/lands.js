...
router.get('/:firebaseUid', requireAuth, async (req, res) => {
  try {
    ...
    if (firebaseUid !== req.firebaseUid) {
      return res.status(403).json({ success: false, message: 'Not authorized to view these lands' });
    }
 ...
 */
router.get('/details/:landId', requireAuth, async (req, res) => {
  try {
    ...
    if (land.firebaseUid !== req.firebaseUid) {
      return res.status(403).json({ success: false, message: 'Not authorized to view this land' });
    }
 ...
 */
router.put('/:landId', requireAuth, async (req, res) => {
  try {
    const { landId } = req.params;
    ...
    const existingLand = await Land.findById(landId);
    if (!existingLand) {
      return res.status(404).json({
        success: false,
        message: 'Land not found'
    ...
    if (existingLand.firebaseUid !== req.firebaseUid) {
 ...
 */
router.delete('/:landId', requireAuth, async (req, res) => {
  try {
    const { landId } = req.params;
