...
// Emoji + accent color per crop for the Market Prices card — purely visual
// styling (not price data), with a sensible fallback for any crop name.
...
const getCropDisplayMeta = (cropName) =>
  CROP_DISPLAY_META[(cropName || '').toLowerCase()] || { emoji: '🌿', color: '#16A34A' };
...
export default function FarmerDashboard({ navigation, route }) {
  ...
  // Real Agmarknet mandi prices for the Market Prices card — driven by
  // whatever crop(s) the farmer actually registered for this land, never
  // hard-coded. Runs once per (land, crop list) change; the backend caches
  // the underlying Agmarknet call, so switching lands and back doesn't
  // re-fetch from Agmarknet every time.
  useEffect(() => {
    if (!selectedLand || crops.length === 0) {
      setMandiPrices([]);
      return;
    }
    loadMandiPrices(selectedLand, crops);
  }, [selectedLand?._id, crops]);

  const loadMandiPrices = async (land, cropList) => {
    try {
      setMandiLoading(true);
      const cropNames = cropList.map((c) => c.name).slice(0, 2).join(',');
      const today = new Date().toISOString().slice(0, 10);
      const res = await axios.get(`${API_ENDPOINTS.MANDI}/dashboard-prices`, {
        params: {
      ...
      console.log('⚠️ Mandi price fetch error (non-critical):', err.message);
      setMandiPrices([]);
    } finally {
  ...
  if (lands.length === 0) {
    return (
      <View style={{ flex: 1 }}>
        ...
        <UzhavanChatbot /> {/* 👈 ADDED LINE 2 — also shown on empty state */}
      </View>
    );
  }
        ...
        <View style={styles.card}>
          ...
          {crops.length === 0 ? (
            <View style={styles.mandiEmptyWrap}>
              ...
              <Text style={styles.mandiEmptyText}>Add a crop to see its mandi price here</Text>
            </View>
          ) : mandiLoading && mandiPrices.length === 0 ? (
            <View style={styles.mandiEmptyWrap}>
              <ActivityIndicator size="small" color="#16A34A" />
              <Text style={styles.mandiEmptyText}>Fetching mandi prices…</Text>
            </View>
          ) : (
            <>
              <View style={styles.marketRow}>
                {mandiPrices.map((item) => {
                  ...
                  return (
                    <View key={item.cropName} style={styles.marketCard}>
...
const styles = StyleSheet.create({
  ...
  marketCard:  { flex: 1, backgroundColor: '#F9FAFB', borderRadius: 12, padding: 12, marginRight: 5 },
