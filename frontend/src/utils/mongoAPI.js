...
export const getUserByFirebaseUid = async (firebaseUid) => {
  try {
    ...
    if (!response.ok) {
      // 404 here is expected right after signup — the Firebase account
      // exists a moment before the MongoDB record does. The caller
      // (RootNavigator) retries, so this isn't a real failure and
      // shouldn't surface as a console error / red screen.
      if (response.status === 404) {
        console.log('ℹ️ User not yet in MongoDB (expected during signup):', firebaseUid);
      } else {
        console.error('❌ API Error:', data.error);
      }
      return {
